...
type UserForModel = Pick<
  ...
  | 'updatedAt'
>;
type ProfileForModel = Pick<Profile, "fullName" | "bio">;
...
export class AuthService {
  ...
  private toMeModel(user: UserForMeModel): MeModel {
    return {
      id: user.id,
      username: user.username,
      email: user.email,
      ...
      profile: this.toProfileModel(user.profile),

      notificationSettings: this.toNotificationSettingsModel(
        user.notificationSettings,
      ),
    };
  }

  private toProfileModel(profile: UserForMeModel['profile']): ProfileModel {
    return {
      id: profile!.id,
      fullName: profile!.fullName,
      ...
      privacySettings: profile!.privacySettings ?? undefined,

      professionalIdentity: profile!.professionalIdentity
        ? {
            currentRole: profile!.professionalIdentity.currentRole ?? undefined,
            company: profile!.professionalIdentity.company ?? undefined,
            industry: profile!.professionalIdentity.industry ?? undefined,
            highestEducation:
  ...
  private async findValidSession(
    sessionId: string,
  ): Promise<SessionWithUserProfile> {
  ...
  async register(input: RegisterInput): Promise<AuthPayload> {
    ...
    const { user, profile, accessToken, refreshToken } =
      await this.createAccount(input, passwordHash);
  ...
  async me(userId: string): Promise<MeModel> {
  ...
  async resetPassword(
    ...
    if (!user) {
      ...
      await argon2
        .verify(await this.getDummyHash(), otp)
        .catch(() => undefined);
